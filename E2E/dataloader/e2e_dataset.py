...
class e2e_dataset(Dataset):
    def __init__(self ,dataset_path,mode , img_height , img_width , applyAllTransforms=False):
        ...
        DenseDir =dataset_path+ '/training/denseDisp/'
        SparseDir =dataset_path+ '/training/DepthGamedAwy/'
        if mode == 'train':
        ...
        for name in filenames:
                ...
                sample['dense'] = DenseDir + name + '.npy'
                sample['sparse'] = SparseDir + name + '.npy'
    ...
    def __getitem__(self, index):
        ...
        sample['disp'] = get_depth(sample_path['sparse']) 
        
        #label

        #sample['label'] = torch.Tensor(read_label(index , self.samples_paths))
        
        sample = self.transform(sample) #5aly balk enta la5et el disp crop
        #print(sample['disp'].shape)
        #print(sample['disp'])
        #print(sample['disp1'])
        #print( sample['disp'].shape )
        #print( sample['disp1'].shape )
        
        #print( sample['disp'].dtype )
        #print( sample['disp1'].dtype )
        #exit()
        return sample
