...
def parse_args():
    ...
    parser.add_argument("--od_cp", default="/notebooks/epoch_15_13040.pth", type=str,
                        help="Loading Weights for Object Detection model")
    parser.add_argument("--depth_cp", default = "/notebooks/E2E/pretrained/dp/E2E_Depth.tar", type = str, help = "Loading Weights for Depth model")
...
def main():
    ...
    test_dataset = e2e_dataset(args.data_dir, 'val', 384, 1280)
    test_loader = DataLoader(dataset=test_dataset, batch_size=int(args.batch_size), shuffle=False,
    ...
    DP_interface = dp_interface(pretrained = args.depth_cp , mode='val')
...
#     CIA_interface.register_aanet(AANET_interface)
    ...
    import time
    for i, sample in enumerate(tqdm(test_loader)):  # note that we enumerate on samples not batches here
        ...
        with torch.no_grad():
            ...
            #t0 = time.time()
            ############################################################################################################
            pred_disp = DP_interface.dp_test_batch(left , right , 90)[0]  ##need to be changed to test batch
