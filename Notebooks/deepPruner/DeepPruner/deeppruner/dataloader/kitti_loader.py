...
class KITTILoader(data.Dataset):
    def __init__(self, training, loader=default_loader, dploader=disparity_loader):
        
        filenames = open("/notebooks/dataset/train.txt").read().split('\n')
        leftImagesDir = '/notebooks/dataset/training/image_2/'
        rightImagesDir =  '/notebooks/dataset/training/image_3/'
        DenseDepthDir = '/notebooks/dataset/training/denseDepth/'
        left_images = []
        right_images = []
        denseDepth = []
        for name in filenames:
            left_images.append(leftImagesDir  + name + '.png')
            right_images.append(rightImagesDir  + name + '.png')
            denseDepth.append(DenseDepthDir + name + '.png')
        #print(left_images)
        self.left_img = left_images
        self.right_img = right_images
        self.left_disp = denseDepth
        self.loader = loader
    ...
    def __getitem__(self, index):
        ...
        w, h = left_img.size
        left_img = left_img.crop((0 , 60 , w-1 , h-1))
        right_img = right_img.crop((0 , 60 , w-1 , h-1))
        left_disp = left_disp.crop((0 , 60 , w-1 , h-1))
        w, h = left_img.size
        ...
        return left_img, right_img, left_disp #,x1 , y1 
