...
writer = SummaryWriter("/notebooks/logs")
...
class TextLoggerHook(LoggerHook):
    ...
    def before_run(self, trainer):
        ...
        self.json_log_path = osp.join('/notebooks', "{}.log.json".format(20))
...
#         if trainer.world_size > 1:
    ...
    def _log_info(self, log_dict, trainer):
        ...
        #print("log dict : \n",log_dict)
        writer.add_scalar('Object Detection Loss',np.array(log_dict['loss']),trainer._iter)#log_dict["iter"]+(log_dict["epoch"]-1)*1630)
        writer.add_scalar('Depth Loss', np.array(log_dict['depth_loss']), trainer._iter)
        writer.add_scalar('Total Loss', np.array(log_dict['total_loss']), trainer._iter)
